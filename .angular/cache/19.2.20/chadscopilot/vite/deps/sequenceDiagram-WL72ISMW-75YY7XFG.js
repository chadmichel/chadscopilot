...
var drawText = __name(function(elem, textData) {
  ...
  if (textData.valign !== void 0 && textData.textMargin !== void 0 && textData.textMargin > 0) {
    switch (textData.valign) {
      ...
      case "bottom":
      case "end":
        yfunc = __name(() => Math.round(textData.y + (prevTextHeight + textHeight + 2 * textData.textMargin) - textData.textMargin), "yfunc");
        break;
    }
  }
...
var drawActorTypeControl = __name(function(elem, actor, conf2, isFooter) {
  ...
  _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, rect.y + r + (isFooter ? 5 : 10), rect.width, rect.height, {
...
var drawActorTypeEntity = __name(function(elem, actor, conf2, isFooter) {
  ...
  _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, rect.y + (!isFooter ? (cy + r - actorY) / 2 : (cy - actorY + r - 5) / 2), rect.width, rect.height, {
    class: `actor ${ACTOR_MAN_FIGURE_CLASS}`
  }, conf2);
  if (!isFooter) {
...
`;
  ...
  actor.rectData = rect;
  _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, g, rect.x, rect.y + (!isFooter ? (rect.height + ry) / 2 : (rect.height + h) / 4), rect.width, rect.height, {
    class: `actor ${ACTOR_BOX_CLASS}`
  }, conf2);
  const lastPath = cylinderGroup.select("path:last-child");
  if (lastPath.node()) {
...
var drawActorTypeBoundary = __name(function(elem, actor, conf2, isFooter) {
  ...
  const bounds2 = actElem.node().getBBox();
  actor.height = bounds2.height + (conf2.sequence.labelBoxHeight ?? 0);
  _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, rect.y + (!isFooter ? radius / 2 + 3 : radius / 2 - 4), rect.width, rect.height, {
    class: `actor ${ACTOR_MAN_FIGURE_CLASS}`
  }, conf2);
  if (!isFooter) {
...
var drawActorTypeActor = __name(function(elem, actor, conf2, isFooter) {
  ...
  const bounds2 = actElem.node().getBBox();
  actor.height = bounds2.height;
  _drawTextCandidateFunc(conf2, hasKatex(actor.description))(actor.description, actElem, rect.x, rect.y + 35, rect.width, rect.height, {
    class: `actor ${ACTOR_MAN_FIGURE_CLASS}`
  }, conf2);
  return actor.height;
}, "drawActorTypeActor");
var drawActor = __name(function(elem, actor, conf2, isFooter) {
  return __async(this, null, function* () {
    switch (actor.type) {
      ...
      case "boundary":
        return yield drawActorTypeBoundary(elem, actor, conf2, isFooter);
      case "control":
        return yield drawActorTypeControl(elem, actor, conf2, isFooter);
      case "entity":
        return yield drawActorTypeEntity(elem, actor, conf2, isFooter);
      case "database":
        return yield drawActorTypeDatabase(elem, actor, conf2, isFooter);
      case "collections":
        return yield drawActorTypeCollections(elem, actor, conf2, isFooter);
      case "queue":
        return yield drawActorTypeQueue(elem, actor, conf2, isFooter);
    }
  });
}, "drawActor");
...
var bounds = {
  ...
  createLoop: __name(function(title = {
    message: void 0,
    wrap: false,
    width: void 0
  }, fill) {
...
var drawMessage = __name(function(diagram2, msgModel, lineStartY, diagObj) {
  return __async(this, null, function* () {
    ...
    if (startx === stopx) {
      if (conf.rightAngles) {
        line = diagram2.append("path").attr("d", `M  ${startx},${lineStartY} H ${startx + common_default.getMax(conf.width / 2, textWidth / 2)} V ${lineStartY + 25} H ${startx}`);
      } else {
...
//# sourceMappingURL=sequenceDiagram-WL72ISMW-75YY7XFG.js.map
