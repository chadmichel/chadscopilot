import { Injectable } from '@angular/core';

export interface EditorInstallation {
  found: boolean;
  path: string;
  cli: string;
}

@Injectable({
  providedIn: 'root',
})
export class CursorService {
  private get electron() {
    return (window as any).electronAPI;
  }

  async findInstallation(): Promise<EditorInstallation> {
    if (this.electron?.cursorFindInstallation) {
      return await this.electron.cursorFindInstallation();
    }
    return { found: false, path: '', cli: '' };
  }

  async open(folderPath: string, cliPath?: string): Promise<boolean> {
    if (this.electron?.cursorOpen) {
      return await this.electron.cursorOpen(folderPath, cliPath);
    }
    return false;
  }
}
